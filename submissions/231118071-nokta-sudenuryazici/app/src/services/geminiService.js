...
export async function generateMultipleDashboardConfigs(metadataPrompt, userRequest) {
  ...
  console.log("🎨 [AI Engine] Dashboard tasarımları sırayla üretiliyor (Rate-limit koruması)...");
  const configs = [];
  
  for (const style of styles) {
    try {
      // Bekleme süresi (Rate limit aşmamak için)
      await new Promise(resolve => setTimeout(resolve, 1000));
      const config = await generateDashboardConfig(metadataPrompt, userRequest, style);
      if (config) configs.push(config);
    } catch (e) {
      console.warn(`[AI Engine] ${style} için üretim başarısız:`, e);
    }
  }

  if (configs.length === 0) {
    console.warn("[AI Engine] Gemini API hatası alındı, sunum için Mock Dashboard üretiliyor...");
    return [{
      title: "Yapay Zeka Analiz Paneli",
      subtitle: "API hatası sebebiyle yerel şablon yüklendi (Sunum Modu)",
      widgets: [
        {
          type: "dynamic_canvas",
          title: "Satış Özeti",
          gridPosition: { colSpan: 4, rowSpan: 1, order: 1 },
          config: { htmlTemplate: "<div style='color: white; padding: 20px; background: rgba(255,255,255,0.1); border-radius: 12px;'><h3>Satışlar: 1.25M ₺</h3><p>Hedefin %112 üzerinde</p></div>" }
        }
      ]
    }];
  }
