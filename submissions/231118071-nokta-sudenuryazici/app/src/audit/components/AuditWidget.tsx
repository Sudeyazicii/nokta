...
import { Bug, FileText, AlertTriangle } from 'lucide-react';
...
export function AuditWidget({ currentScreen }: { currentScreen: string }) {
  ...
  const handleSave = async (noteText: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFailCount(0); // Reset fail counter on success
      ...
      // Otonom STUCK Tespiti: Üst üste 2 FAIL/ROLLBACK
      if (newCount >= 2) {
        alert('STUCK Durumu Algılandı! (2 Cycle FAIL). Uzmana otonom olarak bağlanılıyor...');
        setShowWebRTC(true);
      } else {
        alert('Senkronizasyon hatası! (Fail Count: ' + newCount + '/2)');
      }
    }
  ...
  const triggerSimulateFail = () => {
    const newCount = failCount + 1;
    setFailCount(newCount);
    if (newCount >= 2) {
      alert('STUCK Durumu Algılandı! Uzmana bağlanılıyor...');
      setShowWebRTC(true);
    } else {
      alert('Simulated Fail Count: ' + newCount + '/2');
    }
  };
    ...
    <>
      <div style={{ position: 'fixed', bottom: '20px', right: '20px', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {mode === 'idle' && (
          <>
            <button 
              onClick={triggerSimulateFail}
              title="Kasıtlı STUCK Simülasyonu"
              style={{ 
      ...
      {showWebRTC && (
        <WebRTCCall roomName="nokta-forge-emergency" onClose={() => { setShowWebRTC(false); setFailCount(0); }} />
      )}
    </>
  );
}
